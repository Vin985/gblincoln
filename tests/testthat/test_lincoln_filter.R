...
test_that("Time filter, no start", {
  ...
  expect_error(filter_database(gb_banding, filter))
})
...
test_that("Filter, wrong type", {
  expect_error(filter_database(gb_banding, type="s"))
})
...
test_that("Filter column names", {
  ...
  expect_equal(filter_database(gb_banding, filter1), filter_database(gb_banding, filter2))
})
